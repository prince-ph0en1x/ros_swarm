...
public:
	...
	void Adagio()
	{
		// Find A* path of object with transparent agents.
		// Find A* path of associated agents with other agents transparent.
		// Convert movement sequence to coordinates
		// Time sequence to detect collision
		// If no collision, enter movement sequence with parallel command issue till max time
		// If collision, find alternate path with same cost and makespan; or more cost same makespan
		// If not found, retime moves
...
private:
	...
	void PasDeDeux::actuate(const geometry_msgs::Twist& moveMsg)
	{
		...
		if (moveMsg.linear.x == 0) {
			...
			req.left = moveMsg.linear.x;
			req.right = moveMsg.linear.x;
		}
	...
	bool PasDeDeux::sense(pas_de_deux::Sensor::Request& req, pas_de_deux::Sensor::Response& res)
	{
		...
		std::cout << "Receiving Sensor readings from Agent " << req.id << "\t L : " << req.left << "\t R : " << req.right << std::endl;
