...
const int GRID_A = 16;	// Rotation resolution is 2*PI/GRID_A	// 8-45d 16-22.5d 36-10d  
const int NUM_OBJ = 1;
const int NUM_AGT = 3;
				...
				for (agtrr = 0; agtrr < NUM_AGT; agtrr++) {
					...
					if (agttrail[agtrr].size() == 1 && agttrail[agtrr][0] == SKIP) {
						...
						switch (agttrail[agtrr][0]) {
							case MOVE_E		: translateObject(&agt[agtrr],senseWhlEnc(1-err.pos.x),0);	break;
							case MOVE_N		: translateObject(&agt[agtrr],0,-senseWhlEnc(1-err.pos.y));	break;
							case MOVE_W		: translateObject(&agt[agtrr],-senseWhlEnc(1-err.pos.x),0);	break;
							case MOVE_S		: translateObject(&agt[agtrr],0,senseWhlEnc(1-err.pos.y));	break;
							case TURN_C		: rotateObject(&agt[agtrr],-senseWhlEnc(1-err.ori));		break;
							case TURN_AC	: rotateObject(&agt[agtrr],senseWhlEnc(1-err.ori));			break;
						}
						printDebug();
						std::cout << "\nObject " << n << " ";
